...
def _get_object_world_scale(
    ...
    if not hasattr(env, "_task2_object_world_scale_cache"):
        env._task2_object_world_scale_cache = {}
    ...
    if cache_key in env._task2_object_world_scale_cache:
        scale = env._task2_object_world_scale_cache[cache_key]
    ...
    env._task2_object_world_scale_cache[cache_key] = scale
    return torch.tensor(scale, dtype=dtype)
...
def _get_object_local_visual_center(
    ...
    if not hasattr(env, "_task2_object_local_visual_center_cache"):
        env._task2_object_local_visual_center_cache = {}

    prim_path = _get_object_prim_path(object_entity, env_id=0)
    if prim_path is None:
        return torch.zeros(3, dtype=dtype)
    if prim_path in env._task2_object_local_visual_center_cache:
        cached = env._task2_object_local_visual_center_cache[prim_path]
    ...
    # 部分USD资产会给出“世界量级”的中点（而非局部），这里自动纠正回局部坐标
    try:
        center_tensor = torch.tensor(center, dtype=dtype)
        if torch.linalg.vector_norm(center_tensor) > 0.25:
            object_pos_w, object_quat_w = get_object_detection_pose_w(env, object_entity, env_id=None)
            pos0 = object_pos_w[0].detach().cpu()
            quat0 = object_quat_w[0].detach().cpu().clone()
            quat0[1:] = -quat0[1:]  # conjugate

            world_center = torch.tensor(raw_center, dtype=pos0.dtype)
            delta_world = world_center - pos0
            delta_local_scaled = quat_apply(quat0.unsqueeze(0), delta_world.unsqueeze(0))[0]

            scale0 = _get_object_world_scale(env, object_entity, env_id=0, dtype=delta_local_scaled.dtype).cpu()
            safe_scale0 = torch.where(scale0.abs() > 1e-8, scale0, torch.ones_like(scale0))
            delta_local_unscaled = delta_local_scaled / safe_scale0
            center = (
                float(delta_local_unscaled[0].item()),
                float(delta_local_unscaled[1].item()),
                float(delta_local_unscaled[2].item()),
            )
    except Exception:
        pass

    env._task2_object_local_visual_center_cache[prim_path] = center
...
def create_object_detection_point_visualization(
    ...
    if not hasattr(env, "_task2_detection_point_visualizations"):
        env._task2_detection_point_visualizations = {}

    env._task2_detection_point_visualizations[object_name] = {
...
def update_object_detection_point_visualization(
    ...
    if not USD_AVAILABLE or not hasattr(env, "_task2_detection_point_visualizations"):
        return
    if object_name not in env._task2_detection_point_visualizations:
        return

    vis_info = env._task2_detection_point_visualizations[object_name]
...
def init_task2_visualization(
    ...
    if hasattr(env, "_task2_visualization_created"):
        return
    ...
    cup_color = (1.0, 0.84, 0.0)
    holder_color = (1.0, 0.4, 0.8)
    for object_name in object_names:
        region_name = object_region_map.get(object_name) if object_region_map else None
        point_color = holder_color if region_name == "teaholder_zone" else cup_color
...
def update_task2_visualization(env: ManagerBasedRLEnv | DirectRLEnv) -> None:
    if not hasattr(env, "_task2_detection_point_visualizations"):
        return
    for object_name in tuple(env._task2_detection_point_visualizations.keys()):
