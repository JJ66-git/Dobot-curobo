...
```bash
git clone https://gitee.com/dobot-opensource/Guangdong_Collegiate_Computing_Competition.git
```
...
```bash
cd ~/dobot/Guangdong_Collegiate_Computing_Competition/x-trainer
...
采集完并关闭程序之后会在 `~/dobot/Guangdong_Collegiate_Computing_Competition/x-trainer/datasets` 文件夹中生成一个 `test.hdf5` 的数据文件，保存了刚才采集的所有数据，每条数据按demo_0，demo_1，...的顺序命名。
...
- **多文件**：打开 `dobot/Guangdong_Collegiate_Computing_Competition/x-trainer/scripts/convert/isaaclab2lerobot_xtrainer.py`，在 `convert_isaaclab_to_lerobot` 内将 `hdf5_files` 改为包含多个路径的列表，例如：
...
说明：默认的完整保存路径为 `~/.cache/huggingface/lerobot/sim_xtrainer/<repo_id 最后一段>`，例如 `repo_id='sim_xtrainer/xtrainer_task1'` 时对应目录为 `/home/.cache/huggingface/lerobot/sim_xtrainer/xtrainer_task1`。

#### 第三步：打开终端，进入 leisaac 环境：
...
```bash
  lerobot-train \
  --dataset.root=/home/.cache/huggingface/lerobot/sim_xtrainer/xtrainer_task1 \
  --dataset.repo_id=sim_xtrainer/xtrainer_task1 \
  --policy.type=act \
  --output_dir=/home/dobot/Guangdong_Collegiate_Computing_Competition/x-trainer/ckpts/act_task1 \
  --job_name=act_task1 \
  --policy.device=cuda \
  --batch_size=20 \
  --steps=200000 \
...
python scripts/evaluation/policy_inference.py \
    ...
    --device=cuda \
    --enable_cameras \
    --policy_checkpoint_path="/home/dobot/Guangdong_Collegiate_Computing_Competition/x-trainer/ckpts/act_2/checkpoints/200000/pretrained_model" 
```
...
python scripts/evaluation/policy_scoring.py \
    ...
    --device=cuda \
    --enable_cameras \
    --policy_checkpoint_path="/home/dobot/Guangdong_Collegiate_Computing_Competition/x-trainer/ckpts/act_sort/checkpoints/100000/pretrained_model"
```
